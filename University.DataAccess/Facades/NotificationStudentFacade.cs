...
namespace University.DataAccess.Facades {
    ...
    public class NotificationStudentFacade : AbstractFacade<NotificationStudent> {
        ...
        public bool getByUserIdAndNotificationId(int studentId, int notificationId) {
            return GetContext.NotificationStudents.Where(
                    x => x.Student.Id == (studentId) && x.NotificationId == notificationId)
                .Any();
        }
